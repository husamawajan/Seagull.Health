...
namespace Seagull.Services.Directory
{
    ...
    public partial class CurrencyService : ICurrencyService
    {
        ...
        /// {0} : currency ID
        /// </remarks>
        private const string CURRENCIES_BY_ID_KEY = "Seagull.currency.id-{0}";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        ...
        /// </remarks>
        private const string CURRENCIES_ALL_KEY = "Seagull.currency.all-{0}";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string CURRENCIES_PATTERN_KEY = "Seagull.currency.";
        ...
        public virtual decimal ConvertToPrimaryExchangeRateCurrency(decimal amount, Currency sourceCurrencyCode)
        {
            ...
            if (result != decimal.Zero && sourceCurrencyCode.Id != primaryExchangeRateCurrency.Id)
            {
                ...
                if (exchangeRate == decimal.Zero)
                    throw new SeagullException(string.Format("Exchange rate not found for currency [{0}]", sourceCurrencyCode.Name));
        ...
        public virtual decimal ConvertFromPrimaryExchangeRateCurrency(decimal amount, Currency targetCurrencyCode)
        {
            ...
            if (result != decimal.Zero && targetCurrencyCode.Id != primaryExchangeRateCurrency.Id)
            {
                ...
                if (exchangeRate == decimal.Zero)
                    throw new SeagullException(string.Format("Exchange rate not found for currency [{0}]", targetCurrencyCode.Name));
                result = result * exchangeRate;
            }
            return result;
        }
