...
namespace Seagull.Services.Security
{
    ...
    public partial class PermissionService : IPermissionService
    {
        ...
        /// </remarks>
        private const string PERMISSIONS_ALLOWED_KEY = "Seagull.permission.allowed-{0}-{1}";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string PERMISSIONS_PATTERN_KEY = "Seagull.permission.";
        #endregion
        ...
        public virtual IList<PermissionRecordCategory> GetAllPermissionRecordCategory()
        {
            var query = (from ct in _permissionRecordRepository.Table
                        orderby ct.Category
                        select new PermissionRecordCategory
                        {
                            Category = ct.Category
                        }).Distinct().ToList();
            var permissionsCategory = query.ToList();
            return permissionsCategory;
        }
        public virtual IList<PermissionRecord> GetAllPermissionRecordByCategory(string Category)
        {
            var query = (from pr in _permissionRecordRepository.Table.Where(a => a.Category == Category)
                         orderby pr.Id
                         select pr).ToList();
            var permissionsCategory = query.ToList();
            return permissionsCategory;
        ...
        public virtual IList<int> GetAllPermissionForUserRole(int UserRoleId)
        {
            var data = _userRoleRepository.Table.Where(usr=>usr.Id == UserRoleId).SelectMany(per=>per.PermissionRecords).Select(id =>id.Id).ToList();
            return data;
        }
        public class PermissionRecordCategory
        {
