    ...
    public partial class UserRegistrationService : IUserRegistrationService
    {
        ...
        public virtual UserRegistrationResult RegisterUser(UserRegistrationRequest request)
        {
            ...
            if (registeredRole == null)
                throw new SeagullException("'Registered' role could not be loaded");
        ...
        /// <param name="request">Request</param>
        /// <returns>Result</returns>
        public virtual ChangePasswordResult ChangePassword(ChangePasswordRequest request , bool ChangePass = false)
        {
            ...
            UserPassword GetUserPassword = new UserPassword();
            if (!ChangePass)
                _userService.InsertUserPassword(userPassword);
            else
            {
                GetUserPassword = _userService.GetUserPasswords(userPassword.UserId).FirstOrDefault();
                GetUserPassword.Password = userPassword.Password;
                GetUserPassword.PasswordSalt = userPassword.PasswordSalt;
                _userService.UpdateUserPassword(GetUserPassword);
            }
            //publish event
            _eventPublisher.Publish(new UserPasswordChangedEvent(userPassword));
        ...
        public virtual void SetEmail(User user, string newEmail, bool requireValidation)
        {
            ...
            if (newEmail == null)
                throw new SeagullException("Email cannot be null");

            newEmail = newEmail.Trim();
            string oldEmail = user.Email;

            if (!CommonHelper.IsValidEmail(newEmail))
                throw new SeagullException(_localizationService.GetResource("Account.EmailUsernameErrors.NewEmailIsNotValid"));

            if (newEmail.Length > 100)
                throw new SeagullException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailTooLong"));

            var user2 = _userService.GetUserByEmail(newEmail);
            if (user2 != null && user.Id != user2.Id)
                throw new SeagullException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailAlreadyExists"));
        ...
        public virtual void SetUsername(User user, string newUsername)
        {
            ...
            if (!_userSettings.UsernamesEnabled)
                throw new SeagullException("Usernames are disabled");

            newUsername = newUsername.Trim();

            if (newUsername.Length > 100)
                throw new SeagullException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameTooLong"));

            var user2 = _userService.GetUserByUsername(newUsername);
            if (user2 != null && user.Id != user2.Id)
                throw new SeagullException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
