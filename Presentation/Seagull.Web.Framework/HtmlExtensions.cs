...
namespace Seagull.Web.Framework
{
	...
	{
		public static string RequestVerificationToken(this HtmlHelper helper , string id)
		{
	...
	public static class HtmlExtensions
	{
		...
		public static MvcHtmlString DeleteConfirmation<T>(this HtmlHelper<T> helper, string buttonsSelector) where T : BaseSeagullEntityModel
		{
		...
		public static MvcHtmlString DeleteConfirmation<T>(this HtmlHelper<T> helper, string actionName,
			string buttonsSelector) where T : BaseSeagullEntityModel
		{
		...
		public static MvcHtmlString DeleteConfirmationAngular(this HtmlHelper helper, string actionName, int Id ,string buttonsSelector) 
		{
			if (String.IsNullOrEmpty(actionName))
				actionName = "Delete";

			var modalId = MvcHtmlString.Create(buttonsSelector +  "-delete-confirmation")
				.ToHtmlString();
		...
		public static MvcHtmlString SeagullLabelFor<TModel, TValue>(this HtmlHelper<TModel> helper,
				Expression<Func<TModel, TValue>> expression, bool displayHint = true)
		{
			...
			if (metadata.AdditionalValues.TryGetValue("SeagullResourceDisplayName", out value))
			{
				var resourceDisplayName = value as SeagullResourceDisplayName;
		...
		public static MvcHtmlString SeagullEditorFor<TModel, TValue>(this HtmlHelper<TModel> helper,
		...
		public static MvcHtmlString SeagullEditorAngular(this HtmlHelper html, string expression , bool required , string placeholder = "" , string ngminlength = "" , string ngmaxlength = "")
		{
			var result = new StringBuilder();
			...
			if (required)
			{
				result.AppendFormat(
					  "<div class=\"input-group input-group-required\">{0}<div class=\"input-group-btn\"><span class=\"required\">*</span></div></div>",
					  string.Format("<input type = \"text\" ng-model = \"{0}\"" +
					  "class = \"form-control\" name = \"{0}\" {1} {2} {3} />", expression , required ? "\"required\"" : "" ,
					  !string.IsNullOrEmpty(placeholder) ? string.Format("placeholder = \"{0}\"", placeholder) : "",
					  !string.IsNullOrEmpty(ngminlength) ? string.Format("ng-minlength = \"{0}\"", ngminlength) : "",
					  !string.IsNullOrEmpty(ngmaxlength) ? string.Format("ng-minlength = \"{0}\"", ngmaxlength) : ""));
			}
			else
				result.Append(string.Format("<input type = \"text\" ng-model = \"{0}\"" +
					  "class = \"form-control\" name = \"{0}\" {1} {2} />", expression,
					  !string.IsNullOrEmpty(placeholder) ? string.Format("placeholder = \"{0}\"", placeholder) : "",
					  !string.IsNullOrEmpty(ngminlength) ? string.Format("ng-minlength = \"{0}\"", ngminlength) : "",
					  !string.IsNullOrEmpty(ngmaxlength) ? string.Format("ng-minlength = \"{0}\"", ngmaxlength) : ""));
		...
		public static MvcHtmlString SeagullCheckBoxAngular(this HtmlHelper html, string expression,  string ngtruevalue = "", string ngfalsevalue = "")
		{
			var result = new StringBuilder();

			object htmlAttributes = null;
			htmlAttributes = new { @class = "form-control" };
				result.Append(string.Format("<input type = \"checkbox\" ng-model = \"{0}\"" +
					  "class = \"check-box\" name = \"{0}\" {1} {2} />", expression,
					  !string.IsNullOrEmpty(ngtruevalue) ? string.Format("ng-true-value = \"{0}\"", ngtruevalue) : "",
					  !string.IsNullOrEmpty(ngfalsevalue) ? string.Format("ng-false-value = \"{0}\"", ngfalsevalue) : ""));

			return MvcHtmlString.Create(result.ToString());
		}
		public static MvcHtmlString SeagullHiddenAngular(this HtmlHelper html, string expression, string value = "" )
		{
			var result = new StringBuilder();

			object htmlAttributes = null;
			htmlAttributes = new { @class = "form-control" };
			result.Append(string.Format("<input type = \"hidden\" ng-model = \"{0}\"" +
				  "name = \"{0}\" {1} />", expression,
				  !string.IsNullOrEmpty(value) ? string.Format(" value = \"{0}\"", value) : "" ));

			return MvcHtmlString.Create(result.ToString());
		}
		public static MvcHtmlString SeagullDropDownList<TModel>(this HtmlHelper<TModel> helper, string name,
		...
		public static MvcHtmlString SeagullDropDownListFor<TModel, TValue>(this HtmlHelper<TModel> helper,
		...
		public static MvcHtmlString SeagullTextAreaFor<TModel, TValue>(this HtmlHelper<TModel> helper,
		...
		public static MvcHtmlString SeagullDisplayFor<TModel, TValue>(this HtmlHelper<TModel> helper, Expression<Func<TModel, TValue>> expression)
		{
		...
		public static MvcHtmlString SeagullDisplay<TModel>(this HtmlHelper<TModel> helper, string expression)
		{
			var result = new TagBuilder("div");
			result.Attributes.Add("class", "form-text-row");
