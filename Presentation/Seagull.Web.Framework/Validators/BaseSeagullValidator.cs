...
{
    public abstract class BaseSeagullValidator<T> : AbstractValidator<T> where T : class
    {
        protected BaseSeagullValidator()
        {
            PostInitialize();
        }
        ...
        protected virtual void SetDecimalMaxValue<TObject>(IDbContext dbContext)
        {
            ...
            foreach (var expr in expression)
            {
                RuleFor(expr.Value).IsDecimal(maxValues[expr.Key]).WithMessage(string.Format(localizationService.GetResource("Seagull.Web.Framework.Validators.MaxDecimal"), maxValues[expr.Key] - 1));
            }
        }
    }
}