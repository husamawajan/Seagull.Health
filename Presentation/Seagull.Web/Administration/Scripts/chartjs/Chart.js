	...
	Chart.controllers.doughnut = Chart.controllers.pie = Chart.DatasetController.extend({
		...
		updateElement: function(arc, index, reset) {
			...
			var chart = _this.chart,
				chartArea = chart.chartArea,
				opts = chart.options,
				animatioSeagullts = opts.animation,
				arcOpts = opts.elements.arc,
				...
				circumference = reset && animatioSeagullts.animateRotate ? 0 : arc.hidden ? 0 : _this.calculateCircumference(dataset.data[index]) * (opts.circumference / (2.0 * Math.PI)),
				innerRadius = reset && animatioSeagullts.animateScale ? 0 : _this.innerRadius,
				outerRadius = reset && animatioSeagullts.animateScale ? 0 : _this.outerRadius,
				custom = arc.custom || {},
			...
			if (!reset || !animatioSeagullts.animateRotate) {
				if (index === 0) {
...
module.exports = function(Chart) {
	...
	Chart.controllers.polarArea = Chart.DatasetController.extend({
		...
		updateElement: function(arc, index, reset) {
			...
			var opts = chart.options;
			var animatioSeagullts = opts.animation;
			var arcOpts = opts.elements.arc;
			var custom = arc.custom || {};
			...
			var resetModel = {
				...
				y: centerY,
				innerRadius: 0,
				outerRadius: animatioSeagullts.animateScale ? 0 : scale.getDistanceFromCenterForValue(dataset.data[index]),
				startAngle: animatioSeagullts.animateRotate ? Math.PI * -0.5 : startAngle,
				endAngle: animatioSeagullts.animateRotate ? Math.PI * -0.5 : endAngle,
...
module.exports = function(Chart) {
	...
	helpers.extend(Chart.Controller.prototype, {
		...
		render: function render(duration, lazy) {
			...
			var animatioSeagulltions = this.options.animation;
			if (animatioSeagulltions && ((typeof duration !== 'undefined' && duration !== 0) || (typeof duration === 'undefined' && animatioSeagulltions.duration !== 0))) {
				var animation = new Chart.Animation();
				animation.numSteps = (duration || animatioSeagulltions.duration) / 16.66; //60 fps
				animation.easing = animatioSeagulltions.easing;
				...
				// user events
				animation.onAnimationProgress = animatioSeagulltions.onProgress;
				animation.onAnimationComplete = animatioSeagulltions.onComplete;

				Chart.animationService.addAnimation(this, animation, duration, lazy);
			} else {
				this.draw();
				if (animatioSeagulltions && animatioSeagulltions.onComplete && animatioSeagulltions.onComplete.call) {
					animatioSeagulltions.onComplete.call(this);
				}
			}
			return this;
		},
