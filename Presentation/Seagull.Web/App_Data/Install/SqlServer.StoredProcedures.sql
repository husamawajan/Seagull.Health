CREATE FUNCTION [dbo].[Seagull_splitstring_to_table]
(
    @string NVARCHAR(MAX),
    @delimiter CHAR(1)
)
...
CREATE FUNCTION [dbo].[Seagull_getnotnullnotempty]
(
...
CREATE FUNCTION [dbo].[Seagull_getprimarykey_indexname]
(
...
CREATE FUNCTION [dbo].[Seagull_padright]
(
    @source INT, 
    @symbol NVARCHAR(MAX), 
    @length INT
...
BEGIN
	...
	SELECT CAST(data as int) FROM [Seagull_splitstring_to_table](@CategoryIds, ',')	
	...
	SELECT CAST(data as int) FROM [Seagull_splitstring_to_table](@AllowedUserRoleIds, ',')
	...
	SELECT CAST(data as int) FROM [Seagull_splitstring_to_table](@FilteredSpecs, ',') 
...
BEGIN
	--create catalog
	EXEC('
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''SeagullCommerceFullTextCatalog'')
		CREATE FULLTEXT CATALOG [SeagullCommerceFullTextCatalog] AS DEFAULT')
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[Product]''))
		...
		KEY INDEX [' + dbo.[Seagull_getprimarykey_indexname] ('Product') +  '] ON [SeagullCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[LocalizedProperty]''))
		...
		KEY INDEX [' + dbo.[Seagull_getprimarykey_indexname] ('LocalizedProperty') +  '] ON [SeagullCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductTag]''))
		...
		KEY INDEX [' + dbo.[Seagull_getprimarykey_indexname] ('ProductTag') +  '] ON [SeagullCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
END
GO
...
BEGIN
	...
	--drop catalog
	EXEC('
	IF EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''SeagullCommerceFullTextCatalog'')
		DROP FULLTEXT CATALOG [SeagullCommerceFullTextCatalog]
	')
END
GO
...
BEGIN
	...
	INSERT INTO #FilteredUserRoleIds (UserRoleId)
	SELECT CAST(data AS INT) FROM [Seagull_splitstring_to_table](@UserRoleIds, ',')
    ...
    --get category tree
    ;WITH [CategoryTree]
    AS (SELECT [Category].[Id] AS [Id], dbo.[Seagull_padright] ([Category].[DisplayOrder], '0', @lengthOrder) + '-' + dbo.[Seagull_padright] ([Category].[Id], '0', @lengthId) AS [Order]
        FROM [Category] WHERE [Category].[ParentCategoryId] = 0
        UNION ALL
        SELECT [Category].[Id] AS [Id], [CategoryTree].[Order] + '|' + dbo.[Seagull_padright] ([Category].[DisplayOrder], '0', @lengthOrder) + '-' + dbo.[Seagull_padright] ([Category].[Id], '0', @lengthId) AS [Order]
        FROM [Category]
